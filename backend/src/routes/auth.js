...
router.post('/login', [
  ...
  try {
    ...
    // Find user - simple query without includes to avoid enum issues
    let user;
    try {
    ...
    // Update last login - simple update 
    try {
...
// @route   POST /api/auth/delete-test-users
// @desc    Emergency endpoint to delete test users
// @access  Emergency (password protected)
router.post('/delete-test-users', async (req, res, next) => {
  try {
    const { emergencyPassword } = req.body;
    
    // Emergency password check
    if (emergencyPassword !== 'ChillConnect2024!') {
    ...
    console.log('🗑️  Emergency: Deleting test users...');
    
    // Delete test users with raw SQL to avoid enum issues
    ...
    const deleteResult = await req.prisma.$queryRaw`
    ...
    res.json({
      ...
      deletedRecords: 'Phone: 9258221177, Email: mountainsagegiri@gmail.com'
    });

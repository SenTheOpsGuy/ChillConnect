...
router.post(
  ...
  [
    ...
    body('role')
      .isIn(['SEEKER', 'PROVIDER', 'EMPLOYEE', 'MANAGER', 'ADMIN', 'SUPER_ADMIN'])
      .withMessage('Role must be SEEKER, PROVIDER, EMPLOYEE, MANAGER, ADMIN, or SUPER_ADMIN'),
...
router.post(
  ...
  [
    ...
    body('recipient')
      .optional()
      .notEmpty()
      .withMessage('Recipient required if not using user data'),
  ],
  async (req, res, next) => {
    try {
      const errors = validationResult(req);

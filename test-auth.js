...
const TEST_USERS = {
  ...
  seeker: {
    ...
    password: process.env.TEST_SEEKER_PASSWORD || 'test-password',
    expectedRole: 'SEEKER',
  },
  provider: {
    email: process.env.TEST_PROVIDER_EMAIL || 'test-provider@example.com',
    password: process.env.TEST_PROVIDER_PASSWORD || 'test-password',
    ...
    if (result.data) {
      ...
      if (result.data.success && result.data.token) {
        ...
        if (result.data.user && result.data.user.role === credentials.expectedRole) {
          ...
          console.log(
            `   ⚠️  Role mismatch. Expected: ${credentials.expectedRole}, Got: ${result.data.user?.role}`
          )
        }
...
async function runTests() {
  ...
  const fs = require('fs')
  fs.writeFileSync(
    '/Users/rishovsen/ChillConnect/test-results.json',
